  ...
  template: `
    <div>
      <div class="flex items-center justify-between mb-2">
        <span class="text-xs font-semibold uppercase tracking-wider text-gray-400">WHERE 條件</span>
        <button @click="addCondition"
                class="text-xs px-2 py-1 rounded bg-gray-700 hover:bg-gray-600 text-gray-300">
          + 新增條件
        </button>
      </div>
      <div v-if="where.length === 0" class="text-gray-600 text-xs">無條件（回傳所有資料）</div>
      <div v-for="(cond, idx) in where" :key="idx"
           class="flex items-center gap-2 mb-2">
        <span class="text-gray-500 text-xs w-6 text-right">{{ idx === 0 ? 'WHERE' : 'AND' }}</span>
        <!-- 欄位選擇 -->
