    ...
    <div class="flex flex-col gap-4">
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <span class="text-xs font-semibold uppercase tracking-wider text-gray-400">ORDER BY</span>
          <button @click="addSort"
                  class="text-xs px-2 py-1 rounded bg-gray-700 hover:bg-gray-600 text-gray-300">
            + 新增排序
          </button>
        </div>
        <div v-if="orderBy.length === 0" class="text-gray-600 text-xs">無排序</div>
        <div v-for="(sort, idx) in orderBy" :key="idx"
             class="flex items-center gap-2 mb-2">
          <select :value="sort.column"
                  @change="updateSort(idx, 'column', $event.target.value)"
                  class="bg-gray-800 text-gray-200 text-sm rounded px-2 py-1 border border-gray-700 flex-1">
          ...
          <button @click="removeSort(idx)"
                  class="text-gray-600 hover:text-red-400 text-sm px-1">✕</button>
        </div>
      </div>
      ...
      <div class="flex items-center gap-4">
        <div class="flex items-center gap-2">
          ...
          <input type="number" min="0"
                 ...
                 placeholder="0 = 不限"
                 class="bg-gray-800 text-gray-200 text-sm rounded px-2 py-1 border border-gray-700 w-24" />
        </div>
        <div class="flex items-center gap-2">
