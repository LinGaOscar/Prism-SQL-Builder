...
window.TablePanelComponent = {
  ...
  template: `
    <div class="flex gap-4">
      ...
      <!-- JOIN 模式：各表橫向卡片，各自獨立垂直捲動 -->
      <!-- 外層用 height（非 max-height）才能讓卡片的 flex-1 ul 正確繼承高度並捲動 -->
        ...
        <div v-for="tbl in displayTables" :key="tbl.tableName"
          ...
          <div class="flex items-center gap-1 px-2 py-1.5 bg-zinc-100 dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700 shrink-0">
            ...
            <button @click="selectAll(tbl.tableName)"
                    class="text-[11px] text-indigo-500 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-200 transition-colors px-1.5 py-0.5 rounded hover:bg-indigo-50 dark:hover:bg-indigo-950">全</button>
            <button @click="clearAll(tbl.tableName)"
                    class="text-[11px] text-zinc-400 dark:text-zinc-500 hover:text-red-500 dark:hover:text-red-400 transition-colors px-1.5 py-0.5 rounded hover:bg-red-50 dark:hover:bg-red-950">清</button>
          </div>
          <!-- 欄位清單：flex-1 + overflow-y-auto 實現卡片內部垂直捲動 -->
          <ul class="overflow-y-auto custom-scrollbar flex-1">
            <li v-for="col in tbl.columns" :key="col.name"
                class="flex items-center gap-1.5 px-2 py-1.5 hover:bg-zinc-50 dark:hover:bg-zinc-800/80 transition-colors cursor-pointer border-b border-zinc-100 dark:border-zinc-800 last:border-0"
              ...
              <span v-if="col.isPrimaryKey"
                    class="text-[10px] font-medium px-1 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-800 shrink-0">PK</span>
            </li>
          </ul>
        </div>
      </div>
